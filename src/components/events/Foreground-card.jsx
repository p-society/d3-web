...
const ForegroundCard = ({
  ...
  return (
    <div
      className={`flex items-center gap-8 w-full max-w-[1100px] ${
        isImageLeft ? 'flex-row' : 'flex-row-reverse'
      }`}
    >
      {/* Image Section */}
      <div className="w-[456px] h-80 bg-zinc-300 rounded-3xl relative flex-shrink-0">
        ...
        {isImageLeft ? (
          <>
            {/* Bottom Right Circle */}
            <div className="w-24 h-24 bg-lime-400 rounded-full border-[3px] border-black absolute bottom-[-29.91px] right-[-38.32px]" />
          ...
          <>
            {/* Bottom Left Circle */}
            <div className="w-24 h-24 bg-lime-400 rounded-full border-[3px] border-black absolute bottom-[-29.91px] left-[-38.32px]" />
      ...
      {/* Content Section */}
      <div
        className={`max-w-[630px] flex flex-col gap-9 flex-shrink-0 ${
          isImageLeft ? 'items-start' : 'items-end text-right'
        }`}
      >
        {/* Event Title */}
        <div className={`flex flex-col gap-2.5 ${isImageLeft ? 'items-center' : 'items-end'}`}>
          <h2
            className={`text-black font-bold text-4xl font-['ADLaM_Display'] leading-[48px] ${
              isImageLeft ? '' : 'text-right'
            }`}
          >
            {eventName}
          </h2>
          <div
            className={`w-60 h-0 outline-3 outline-offset-[-2px] outline-lime-400 rounded-full ${
              isImageLeft ? 'self-center' : 'self-end'
            }`}
          />
        </div>
        ...
        {/* Description */}
        <p
          className={`max-w-[610px] text-black text-xl font-['Cascadia_Code'] lowercase ${
            isImageLeft ? '' : 'text-right'
          }`}
        >
          {eventDescription}
